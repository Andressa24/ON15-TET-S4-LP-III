...
function buscarPorTitulo(entrada) {
    let filmeEncontrado = "" // ou [] para atribuir um retorno múltiplo
    entrada = entrada.toLowerCase()
    data.forEach(filme => {
        let titulo = filme.titulo.toLowerCase()
        if (titulo.includes(entrada)) {
            filmeEncontrado = filme // inserir push para atribuir um retorno múltiplo
        }       
    })
...
function buscarPorGenero(entrada) {
    ...
    for (let filme of data) {
        if(typeof(filme.genero) == "string") {
            ...
            !!filmeEncontrado ? listaPorGenero.push(filme) : null // !! é para negar a negação. Se for verdadeiro mesmo
        } else {
...
function buscarFilmes(entrada) {
    if (typeof(entrada) == "string") {
        ...
        if(porTitulo.length > 0 && porGenero.length > 0) {
            ...
            console.log("Não foi encontrado nenhum filme com o dado inserido.")
        }
    } else {
